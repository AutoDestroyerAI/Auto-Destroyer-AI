class Destroyer {
    constructor(canvas) {
        ...
        // Initialize Destroyer body
        this.Destroyer = [{x: startX, y: startY}];
    ...
    generateFood() {
        ...
        do {
            ...
            this.Destroyer.some(segment => segment.x === food.x && segment.y === food.y) ||
    ...
    update() {
        // Update Destroyer position
        const head = {
            x: this.Destroyer[0].x + this.direction.x,
            y: this.Destroyer[0].y + this.direction.y
        };
        ...
        // If food is eaten, don't remove tail (Destroyer grows)
        if (!foodEaten) {
            this.Destroyer.pop();
        }

        // Add new food every 15 seconds when Destroyer length is less than 10
        const now = Date.now();
        if (this.Destroyer.length < 10 && now - this.lastFoodTime >= this.FOOD_ADD_INTERVAL) {
    ...
    draw() {
        ...
        this.Destroyer.forEach((segment, index) => {
            this.ctx.fillRect(
    ...
    lookInDirection(direction, type) {
        ...
        while (true) {
            ...
            if (type === 'self' && this.Destroyer.some(s => s.x === pos.x && s.y === pos.y)) {
                return 1/distance;
            }
        }
        return 0;
    ...
    isDead() {
        ...
        for (let i = 1; i < this.Destroyer.length; i++) {
            if (head.x === this.Destroyer[i].x && head.y === this.Destroyer[i].y) {
                return true;
            }
        }
    ...
    setDirection(direction) {
        // Prevent Destroyer from moving in opposite direction
        const opposites = {
            'up': 'down',
            'down': 'up',
            'left': 'right',
